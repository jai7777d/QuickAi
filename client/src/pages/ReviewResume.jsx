    ...
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <input
          ...
          className='w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300 text-gray-600'  // ✅ fixed
          required
        />
      ...
      <div className='w-full max-w-lg p-4 bg-white rounded-lg flex flex-col border border-gray-200 min-h-96 max-h-[600px]'>
        ...
        {content ? (
          <div className='mt-4 overflow-y-scroll'>
            <Markdown className='prose prose-sm max-w-none'>{content}</Markdown>  {/* ✅ fixed */}
          </div>
        ) : (
          <div className='flex-1 flex justify-center items-center'>
            <div className='text-sm flex flex-col items-center gap-5 text-gray-400'>
              <FileText className='w-10 h-10 '/>
              <p>Upload the resume and click "Review Resume"</p>
            </div>
          </div>
        )}
      </div>
